...
const QuickSaleSettingModal = forwardRef(({ visible, club, onRequestClose }, ref) => {
  ...
  const getDescriptorChanges = () => {
    ...
    const added = currentItems.filter(
      (currentItem) =>
        !currentItem.id ||
        !originalItems.some((originalItem) => originalItem.id === currentItem.id),
    );
    ...
    const updated = currentItems.filter((currentItem) => {
      ...
      return (
        ...
        Boolean(originalItem.is_default) !== Boolean(currentItem.is_default) ||
    ...
    try {
      ...
      if (added.length > 0) {
        ...
        if (addResponse?.status !== "success") {
          throw new Error(addResponse?.message || "Failed to add descriptors.");
        }
      }
      ...
      if (updated.length > 0) {
        ...
        if (updateResponse?.status !== "success") {
          throw new Error(updateResponse?.message || "Failed to update descriptors.");
        }
      }
      ...
      if (deleted.length > 0) {
        ...
        if (deleteResponse?.status !== "success") {
          throw new Error(deleteResponse?.message || "Failed to delete descriptors.");
        }
      }
  ...
  const handleDeleteDescriptor = (text_input_id) => {
    ...
    setDescriptors((prev) => prev.filter((item) => item.text_input_id !== text_input_id));
  };

  const handleTextChange = (text_input_id, newText) => {
    setDescriptors((prev) =>
      prev.map((item) =>
        item.text_input_id === text_input_id ? { ...item, name: newText } : item,
      ),
    );
  };
                ...
                {descriptors.map((item) => (
                  <View key={item.text_input_id} style={styles.qsInputRow}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => handleTextChange(item.text_input_id, text)}
                    />
                    ...
                    {!item.isFixed ? (
                      <TouchableOpacity
                        ...
                        onPress={() => handleDeleteDescriptor(item.text_input_id)}
                      >
